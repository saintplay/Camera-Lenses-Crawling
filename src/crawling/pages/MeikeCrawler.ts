...
export class MeikeCrawler extends PageCrawler {
	crawlLensDescription(): Partial<CrawleableLensDescription> {
		...
		const descriptionTexts = CrawlingElements
		...
		const focalLength2 = descriptionTexts
		...
		const focalLength = (CrawlingBase
			.useFirst([focalLength1, focalLength2], CrawlingText) as CrawlingText)
			// Sample: 50mm
		...
		const apertureRange2 = descriptionTexts
			...
			.findByRegExp(/^Aperture (?:scale|range):/gmi)
		...
		const filterSize2 = descriptionTexts
			...
			.findByRegExp(/^Filter (?:Size|diameter|thread):/gmi)
			.extractWithRegExp(/^Filter (?:Size|diameter|thread):\s*(.+)$/gmi, 1)
		...
		const weightGR2 = descriptionTexts
			...
			// Sample: Weight: 176g
			.findByRegExp(/^(?:Product )?Weight:/gm)
			.extractWithRegExp(/^(?:Product )?Weight:\s*(.+)$/gm, 1)
